def merge(x,y):
    ...
    for n in range(len(shorter_list)):
        while long_list_index < len(longer_list) and longer_list[long_list_index] < shorter_list[n]:
            merged__sorted_list.append(longer_list[long_list_index])
            long_list_index += 1 

        merged__sorted_list.append(shorter_list[n])

    for n in range(long_list_index, len(longer_list)):
        merged__sorted_list.append(longer_list[n])

    return merged__sorted_list
